...
func instrument(pkg, shortName, fullName string, fset *token.FileSet, parsedFile *ast.File, info *types.Info, out io.Writer, lits map[Literal]struct{}, blocks *[]CoverBlock, sonar *[]CoverBlock) {
	...
	if lits != nil {
		// Try to detect whether this file was generated by stringer.
		// Ignore it if so.
		if !isStringer(file.astFile) {
...
func isStringer(file *ast.File) bool {
	var stringer bool
	ast.Inspect(file, func(n ast.Node) bool {
		...
		for _, spec := range d.Specs {
			...
			for _, n := range v.Names {
				if strings.HasPrefix(n.Name, "_Op_name") || strings.HasPrefix(n.Name, "_Tag_name") {
					stringer = true
					return false
				}
			}
